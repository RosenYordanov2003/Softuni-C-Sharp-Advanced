...
namespace OnlineShop.Core
{
    public class CommandInterpreter : ICommandInterpreter
    {
        public string ExecuteCommand(string[] data, IController controller)
        {
            ...
            if (commandType == CommandType.AddComputer)
                msg = AddComputer(clearData, controller);
            else if (commandType == CommandType.AddPeripheral)
                msg = AddPeripheral(clearData, controller);
            else if (commandType == CommandType.RemovePeripheral)
                msg = RemovePeripheral(clearData, controller);
            else if (commandType == CommandType.AddComponent)
                msg = AddComponent(clearData, controller);
            else if (commandType == CommandType.RemoveComponent)
                msg = RemoveComponent(clearData, controller);
            else if (commandType == CommandType.BuyComputer)
                msg = BuyComputer(clearData, controller);
            else if (commandType == CommandType.BuyBestComputer)
                msg = BuyBestComputer(clearData, controller);
            else if (commandType == CommandType.GetComputerData)
                msg = GetComputerData(clearData, controller);
            else if (commandType == CommandType.Close) Environment.Exit(0);
